...
if (!token) {
  console.error(
    "Environment variable BOT_TOKEN is not set. Add it to .env or system variables.",
  );
  process.exit(1);
}
...
const generateScene = new Scenes.WizardScene(
  ...
  async (ctx) => {
    ...
    // Предлагаем выбрать тип документа кнопками (inline keyboard)
    await ctx.reply(
      "📝 Начнём создание документа.\n\n" + "Выберите тип документа:",
      {
        reply_markup: {
          inline_keyboard: [
            [
              { text: "Доклад (0)", callback_data: "type_0" },
              { text: "Сообщение (1)", callback_data: "type_1" },
              { text: "Реферат (2)", callback_data: "type_2" },
            ],
          ],
        },
      },
  ...
  async (ctx) => {
    // Поддерживаем выбор типа через inline-кнопки (callback_query) или через текстовое сообщение
    let type = null;

    // Если пользователь нажал кнопку
    if (
      ctx.callbackQuery?.data &&
      String(ctx.callbackQuery.data).startsWith("type_")
    ) {
      const payload = String(ctx.callbackQuery.data);
      type = Number(payload.split("_")[1]);
      // подтверждаем callback чтобы убрать "часики" в UI
      try {
      ...
      if (text === undefined) {
        await ctx.reply(
          "❌ Пожалуйста, отправьте число 0, 1 или 2.\n\nПопробуйте снова:",
        );
        return;
      }

      type = text === "" ? 0 : Number(text);
    }

    if (!Number.isInteger(type) || type < 0 || type > 2) {
      await ctx.reply(
        "❌ Неверный формат. Укажите 0, 1 или 2.\n\nПопробуйте снова:",
      );
      return;
    }
    ...
    await ctx.reply(
      '📌 Введите название темы (без символов \\ / : * ? " < > |).',
    );
    return ctx.wizard.next();
  },
  ...
  async (ctx) => {
    ...
    if (/[\\/:*?"<>|]/.test(text)) {
      await ctx.reply(
        '❌ Тема содержит запрещённые символы \\ / : * ? " < > |.\n\nПопробуйте снова:',
      );
      return;
    }
  ...
  async (ctx) => {
    ...
    // Показываем список готовых фамилий в виде inline-кнопок + опция "Другая"
    {
      const intro =
        "👨‍🏫 Выберите фамилию преподавателя из списка или введите полное ФИО вручную.\n" +
        "Если введенной фамилии нет в списке, потребуется дополнительная информация.";

      // Формируем клавиатуру: по 3 фамилии в ряд, callback_data содержит индекс
      const inline = [];
      let row = [];
      ...
      if (row.length) inline.push(row);
      // Добавим кнопку "Другая" для ручного ввода
      inline.push([{ text: "Другая фамилия", callback_data: "teacher_other" }]);
  ...
  async (ctx) => {
    // Поддерживаем выбор преподавателя через inline-кнопки или ручной ввод
    let text = null;
    // Если нажата inline-кнопка
    if (ctx.callbackQuery?.data) {
      const data = String(ctx.callbackQuery.data);
      // выбор из списка фамилий
      if (data.startsWith("teacher_")) {
        const payload = data.slice("teacher_".length);
        if (payload === "other") {
          // пользователь выбрал "Другая фамилия" -> просим вручную ввести фамилию / ФИО
          try {
            await ctx.answerCbQuery();
          } catch (e) {}
          await ctx.reply(
            "Введите фамилию преподавателя или полное ФИО (ручной ввод):",
          );
          return; // остаёмся на этой же стадии, ожидаем текст
        }
        // payload теперь индекс — берём фамилию по индексу из teacherNames
        const idx = Number(payload);
        if (!Number.isNaN(idx) && idx >= 0 && idx < teacherNames.length) {
          text = teacherNames[idx];
        } else {
          // Некорректный индекс — подтверждаем callback и просим ввести вручную
          try {
            await ctx.answerCbQuery();
          } catch (e) {}
          await ctx.reply(
            "Некорректный выбор. Пожалуйста, введите фамилию преподавателя вручную:",
          );
          return;
        }
        try {
    ...
    // Если нет выбора через callback, читаем текстовое сообщение
    if (text === null) {
    ...
    // Проверяем, есть ли введенная фамилия в списке (точное совпадение)
    ...
    if (foundTeacherName) {
      ...
      await ctx.reply(
        "📝 Введите первую строку должности/регалий преподавателя:",
      );
    }
  ...
  async (ctx) => {
    ...
    if (inputMode === 1) {
      ...
      if (!text) {
        await ctx.reply(
          "❌ ФИО курсанта не может быть пустым.\n\nПопробуйте снова:",
        );
        return;
      }
      ...
      try {
        ...
        if (error instanceof Error) {
          if (
            ...
            message =
              "Превышено время ожидания. Попробуйте позже или обратитесь к администратору.";
          } else {
      ...
      if (!text) {
        await ctx.reply(
          "❌ Первая строка должности не может быть пустой.\n\nПопробуйте снова:",
        );
        return;
      }
      ctx.wizard.state.data.teacherLine1 = text;
      await ctx.reply(
        "📝 Введите вторую строку должности/регалий преподавателя:",
      );
      return ctx.wizard.next();
    }
  },
  ...
  async (ctx) => {
    ...
    if (!text) {
      await ctx.reply(
        "❌ Вторая строка должности не может быть пустой.\n\nПопробуйте снова:",
      );
      return;
    }
...
bot.start(async (ctx) => {
  // Добавляем удобную кнопку для запуска создания документа
  await ctx.reply(
    "👋 Привет! Я бот для генерации документов.\n\n" +
      "Нажмите кнопку ниже, чтобы начать создание документа.\n" +
      "Также доступны команды: /generate и /cancel",
    {
      reply_markup: {
        keyboard: [[{ text: "/generate" }]],
...
bot.catch((err, ctx) => {
  ...
  if (
    ...
    if (ctx && ctx.reply) {
      ctx
        .reply(
          "❌ Произошла ошибка: превышено время ожидания. Попробуйте позже.",
        )
        .catch(console.error);
    }
  }
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      await Promise.race([
        ...
        new Promise((_, reject) => {
          setTimeout(() => {
            reject(
              new Error(
                "ETIMEDOUT: Превышено время ожидания отправки документа",
              ),
            );
          }, timeoutMs);
        }),
      ...
      if (isTimeout && attempt < maxRetries) {
        console.log(
          `Попытка ${attempt} отправки документа не удалась (timeout), повторяю...`,
        );
