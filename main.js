...
export default async function mainGen() {
  ...
  for (let i = 0; i < request.length; i++) {
    ...
    if (request[i].length <= 5) {
      teacherName = teachers[request[i][3 + n]];
      line1 = teachers[request[i][3 + n]][1];
      line2 = teachers[request[i][3 + n]][2];
    } else {
      teacherName = request[i][3 + n];
      line1 = request[i][4 + n];
      line2 = request[i][5 + n];
    }

    let kursant = request[i][1 + n];
    let group = request[i][2 + n];
    let theme = request[i][0 + n];

    let IsBroken = false;

    let prompt = `Напиши сообщение на тему "${theme}". Текст должен состоять из содержания, введения, основной части, заключения и списка литературы. Должны быть разрывы страниц между введением, основной частью, заключением и списком литературы. Уложись в 6 страниц листа А4 минимум.`;
    ...
    if (!IsBroken) {
      ...
      try {
        fs.writeFileSync(`./outputdocs/${docParts[1]}.docx`, outputzip, {
          encoding: "base64",
        });
      } catch (error) {
