...
for (let i = 0; i < request.length; i++) {
  ...
  if (!IsBroken) {
    ...
    try {
      ...
      console.log(`записываю так ${docParts[1].slice(0, 30)}`);
      fs.writeFileSync(
        `./outputdocs/${docParts[1].slice(0, 30)}.docx`,
        outputzip,
        { encoding: "base64" },
      );
      console.log(
        `\nВаш итоговый ${type} (${i + 1}): "${docParts[1].slice(0, 30)}.docx" лежит по данному пути: \n /DocGenPC/outputdocs/${docParts[1]}.docx\n`,
      );
    }
  }
}
